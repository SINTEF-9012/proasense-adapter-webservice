    ...
    public void run() {
        while (true) {
            try {
                ...
                // Set new XML start date (last retrieval date)
                startDate.setTime(endDate.getTime());
                xmlStartDate = DatatypeFactory.newInstance().newXMLGregorianCalendar(startDate);

                // Wait poll interval (minutes)
                TimeUnit.MINUTES.sleep(this.pointConfig.getPollInterval());

                // Set new XML end date (current time)
                endDate = new GregorianCalendar();
                xmlEndDate = DatatypeFactory.newInstance().newXMLGregorianCalendar(endDate);
            }
